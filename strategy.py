...
CODE_VERSION = "PATCH24_2026-04-22"
...
def evaluate_entry(sym, price, size, prices_series, sizes_series, ts_val,
    ...
    if not pd.isna(vwap_val) and vwap_val > 0:
        ...
        # PATCH21: regime-aware VWAP extension — TREND on bull days legitimately stays above VWAP
        ...
        if _vwap_extension > _vwap_ext_max:
            logging.debug(
                ...
                sym, _vwap_extension, _vwap_ext_max
            )
    ...
    if regime == "TREND":
        ...
        # PATCH24: SPY context scoring from consolidated states
        _spy_mom_score  = globals().get("SPY_MOMENTUM", "FLAT")
        _spy_bias_score = globals().get("SPY_DAY_BIAS", "NEUTRAL")

        if _spy_mom_score == "RISING":
            score += 0.3
            signal_stack["spy_context"] = "+0.3 (SPY RISING)"
        elif _spy_mom_score == "FADING":
            score -= 1.0
            signal_stack["spy_context"] = "-1.0 (SPY FADING)"
        else:
            signal_stack["spy_context"] = "0.0 (SPY FLAT)"

        if _spy_bias_score == "BULL":
            score += 0.15
            signal_stack["day_bias_score"] = "+0.15 (BULL)"
        elif _spy_bias_score == "BEAR":
            score -= 0.3
            signal_stack["day_bias_score"] = "-0.3 (BEAR)"
        else:
            signal_stack["day_bias_score"] = "0.0 (NEUTRAL)"

        logging.debug(
            "[PATCH24][TREND_SCORE][%s] momentum=%s bias=%s "
            "final_score=%.2f threshold=%.2f",
            sym, _spy_mom_score, _spy_bias_score,
            score, adaptive_entry_threshold(CONFIG, sym, regime)
        )
...
def evaluate_sell(
    ...
    try:
        ...
        # === PATCH24: exit threshold multiplier from consolidated states ===
        _spy_risk_exit = globals().get("SPY_RISK", "NORMAL")
        _spy_mom_exit  = globals().get("SPY_MOMENTUM", "FLAT")
        _vol_multiplier = 1.0

        if _spy_risk_exit == "EXTREME":
            _vol_multiplier = 2.0
            logging.debug("[PATCH24][EXIT][%s] SPY_RISK EXTREME — widening x2.0", sym)
        elif _spy_risk_exit == "ELEVATED":
            _vol_multiplier = 1.5
            logging.debug("[PATCH24][EXIT][%s] SPY_RISK ELEVATED — widening x1.5", sym)

        if _spy_mom_exit == "FADING":
            _vol_multiplier = max(0.5, _vol_multiplier * 0.6)
            logging.debug(
                "[PATCH24][EXIT][%s] SPY FADING — tightening exits to x%.2f",
                sym, _vol_multiplier
            )
