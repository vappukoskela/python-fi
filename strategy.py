...
def safe_market_buy(trade_client_local, symbol, cash_amount, order_lock):
    with order_lock:
        try:
            ...
            logging.info("[TRADE] %s - BUY submitted qty=%d (est_price=%s cash=%.2f) | Time=%s", symbol, qty, str(est_price), cash_amount, datetime.now(timezone.utc).strftime("%H:%M:%S"))
...
def main():
    ...
    if RUN_MODE in ["SIM", "AGG_SIM"]:
        ...
        start = "2025-11-26T14:30:00Z"
        end = "2025-11-26T21:00:00Z"
